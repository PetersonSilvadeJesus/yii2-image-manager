var imageManagerInput = {
    ...
    initModal: function () {
        ...
        console.log($("#modal-imagemanager").length);
        if ($("#modal-imagemanager").length === 0) {
            //set html modal in var
            var sModalHtml = '<div tabindex="-1" role="dialog" class="fade modal" id="modal-imagemanager" aria-hidden="true">';
            ...
            sModalHtml += '<button aria-hidden="true" data-dismiss="modal" class="close" type="button"><span aria-hidden="true">&times;</span></button>';
            sModalHtml += '</div>';
    ...
    openModal: function (inputId, aspectRatio, cropViewMode) {
        ...
        $("#modal-imagemanager .modal-dialog .modal-header .modal-title").text(imageManagerInput.message.imageManager);
        //open modal
